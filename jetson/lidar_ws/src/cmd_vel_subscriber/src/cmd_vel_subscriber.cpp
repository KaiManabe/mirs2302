/*
ROSからの速度指令値(cmd_vel)をソケット通信でraspiに送信する
raspiがこれを受け取ってシリアル通信でarduinoに転送する
*/
...
/*
新しいcmd_velトピックを受け取るたびに呼ばれるコールバック関数
ソケット通信で指令値を送信している
データ通信量を減らすためにへんな変換をして3bytesに圧縮している
*/
...
int main(int argc, char** argv){
    ...
    while(1){
        ...
        {   
            ...
            */
            if(errno != 115){
                printf("[WARN][cmd_vel_pub] : サーバに接続できませんでした。リトライ中...\n");
                usleep(5000000);
                retries += 1;
                if (retries > 5){
                    printf("[FATAL][cmd_vel_pub] : サーバに接続できませんでした。ノードを終了します\n");
                    close_sock(-1);
                    return -1;
                }
            }
    ...
    */
    ...
    /*
    rosノードを初期化、cmd_velをsubscribeしてコールバック関数を設定
    */
